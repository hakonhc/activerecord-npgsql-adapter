  ...
  module ConnectionAdapters
    ...
    class PostgreSQLAdapter < AbstractAdapter
      ...
      def quote_column_name(name) #:nodoc:
        b = Npgsql::NpgsqlCommandBuilder.new
        b.QuoteIdentifier(name.to_s)
      end
      ...
      def result_as_array(reader) #:nodoc:
        # check if we have any binary column and if they need escaping
        unescape_col = []
        for j in 0...reader.FieldCount do
          # unescape string passed BYTEA field (OID == 17)
          unescape_col << ( reader.GetDataTypeOID(j).to_i == 17 )          
        end          
				rows = []
				while reader.read
					row = []
      ...
      def query(sql, name = nil) #:nodoc:
        log(sql, name) do
          cmd = Npgsql::NpgsqlCommand.new(sql,@connection)          	
      ...
      def tables(name = nil)
        ActiveRecord::Base.logger.debug "path:" + schema_search_path        
        ActiveRecord::Base.logger.debug schema_search_path.class              
        
        tmp = schema_search_path.split(/,/)
        schemas = tmp.map { |p| quote(p) }.join(',')
      ...
      def columns(table_name, name = nil)
        ...
        a = column_definitions(table_name).collect do |name, type, default, notnull|
      ...
      protected
        ...
        def postgresql_version
          @postgresql_version ||=
             @connection.server_version =~ /(\d+)\.(\d+)\.(\d+)/
      ...
      private
        ...
        def connect
          ...
          @async = false #@config[:allow_concurrency] && @connection.respond_to?(:async_exec)
        ...
        def select(sql, name = nil)
          ...
          for row in rows
            ...
            fields.each_with_index do |f, i|
              val = row[i]
              row_hash[f] = val.respond_to?(:rstrip) ? val.rstrip : val              
            end
            result << row_hash
          end          
          result
						...
						begin
							...
							if schema != nil
								for row in schema.rows
									fields << row.item(0).to_s
								end
							end
							results = result_as_array(reader)  					   					
